...
resource "azurerm_key_vault_secret" "openai_key" {
  count        = var.skip_key_vault_secret ? 0 : 1
  name         = "openai-api-key"
  value        = azurerm_cognitive_account.openai.primary_access_key
  key_vault_id = var.key_vault_id
}

# Use a local value for the API key to handle both cases
locals {
  api_key = var.skip_key_vault_secret ? azurerm_cognitive_account.openai.primary_access_key : azurerm_key_vault_secret.openai_key[0].value
}
